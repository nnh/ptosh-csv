...
for (i in 1:length(diff_list)){
  ...
  if(class(res) != "try-error") {
    ...
    # "試験名"
    output_csv$試験名 <- input_csv$試験名
    input_csv <- input_csv[ ,colnames(input_csv) != "試験名"]
    # "シート名"
    output_csv$シート名 <- NA
    input_csv <- input_csv[ ,colnames(input_csv) != "シート名"]
    # "シート作成時施設名"
    ...
    # "生年月日"
    output_csv$生年月日 <- input_csv$生年月日
    ...
    # "患者イニシャル"
    output_csv$患者イニシャル <- input_csv$患者イニシャル
    input_csv <- input_csv[ ,colnames(input_csv) != "患者イニシャル"]
    # "患者カナ"
    output_csv$患者カナ <- input_csv$患者カナ
    ...
    # "生死"
    output_csv$生死 <- input_csv$生死
    input_csv <- input_csv[ ,colnames(input_csv) != "生死"]
    # "死亡日"
    output_csv$死亡日 <-  input_csv$死亡日
    input_csv <- input_csv[ ,colnames(input_csv) != "死亡日"]
    # "最終確認日"
    output_csv$最終確認日 <-  input_csv$最終確認日
    ...
    # csv出力
    optnm <-  gsub(constbef, constaft, diff_list[i])
    write.csv(output_csv, paste(diffeditpath, optnm, sep="/"), na='""', row.names=F)
